...
export const EditForm = ({ data, updateUi, show, brandsData, notify }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        show(false)
        database.updateProduct(data.id, updatedProduct, notify, show);
        updateUi();
    ...
    return (
        <form className="p-5 bg-white rounded-md" onSubmit={handleSubmit}>
            ...
            <div className="mt-4">
                ...
                <textarea name="description" value={product.description} onChange={handleInputChange} placeholder="Description" className="w-full mb-2 px-3 py-2 border rounded min-h-[150px]"></textarea>
                ...
                <button type="submit" onClick={handleSubmit} className="mt-4 bg-blue-600 text-white px-4 py-2 rounded">Save Product</button>
            </div>
        </form>
    );
};
